...
from rosa.lib.config import build_tools_map
from rosa.lib.executor import exec_tool
from rosa.lib.logger import LOG
from rosa.lib.utils import find_files, get_env
...
def java_build(src, reports_dir, lang_tools):  # scan:ignore
    ...
    if pom_files:
        ...
        LOG.info("Build the project using bazel build command first")
        return False
    if not cmd_args:
        LOG.info(
...
def android_build(src, reports_dir, lang_tools):  # scan:ignore
    ...
    if gradle_files or gradle_kts_files:
        ...
        LOG.info("Build the project using bazel build command")
        return False
    cp = exec_tool("auto-build", cmd_args, src, env=env, stdout=subprocess.PIPE)
    if cp:
        LOG.debug(cp.stdout)
